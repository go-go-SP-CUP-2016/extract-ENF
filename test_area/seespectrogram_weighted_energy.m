...
for i=1:numfids% different grid
    ...
    %for knob=2.^value
        ...
        ss=spectrogram(y_fil,10000,5,1600,1000);
        
        energy=abs(ss).*abs(ss);
        ...
        filenames = strrep(cell2mat(fileList(i)), '.wav', '.jpg');

        saveas(fig,filenames);
        
        fft_f_estimate=fft(abs(f_estimate));
        
        fig=figure
        plot(abs(fft_f_estimate));

                
        title([cell2mat(fileList(i)), 'fft'])
        filenames = strrep(cell2mat(fileList(i)), '.wav', 'fft_.jpg');
